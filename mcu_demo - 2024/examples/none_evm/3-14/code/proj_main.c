...
{
	 //自己加
	 //uint32_t gpio_status_reg = ool_read32(PMU_GPIO_STATUS_REGISTER); // 读取 GPIO 状态寄存器
	 uint32_t pmu_int_pin_setting = ool_read32(PMU_REG_PORTA_TRIG_MASK); 
	// 读取 PMU_REG_PORTA_TRIG_MASK 寄存器的值
	...
	// 将 gpio_value 的值写入 PMU_REG_PORTA_LAST 寄存器
	
	
	uint32_t tmp = gpio_value & pmu_int_pin_setting;
	// 对 gpio_value 和 pmu_int_pin_setting 进行位运算
...
void element_init(void)
{
 ...
 pmu_set_pin_pull(GPIO_PORT_C,(1<< GPIO_BIT_7),true);//霍尔
...
{    
    ...
    NVIC_EnableIRQ(PMU_IRQn);
	  ...
	  LCD_Fill(0,0,LCD_W,LCD_H,WHITE);
		...
		//system_set_port_pull(GPIO_PC5, true);//按钮
 		
    system_set_port_pull(GPIO_PB7, true);//使能nb
		element_init(); //霍尔初始化,用的mpu加中断
