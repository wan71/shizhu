...
//uint8_t at_c[]="+CDB:123";

extern uint8_t uint8_imei[15];
...
#if 1
__attribute__(()) __attribute__((section("ram_code"))) void uart1_isr_ram(void)
{
    uint8_t int_id;
    volatile uint8_t c;
...
__attribute__(()) __attribute__((section("ram_code"))) void uart0_isr_ram(void)
{
    uint8_t int_id;
    uint8_t c;
    ...
    if(int_id == 0x04 || int_id == 0x0c )   /* Receiver data available or Character time-out indication */
    {
        ...
        ci=0;
				co_printf("%s\r\n",rev);
				at_uart_init((char *)rev);
					
				//at_uart(NULL);//触发任务
				memset(rev,'\0',sizeof(rev));	
				}					

				//用if不断去和rev比，一开始肯定不对，但后面对了就输出并清空rev	
